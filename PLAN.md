...
### 五·四、需求验证(2026-08-28,全部原始数据:repo API/issue 队列/npm 下载量)

**基本面**:dsh 本体 **200,671⭐**、@deepseek-ai/dsh-web-app 周下载 **47.7 万**;@jackwener/opencli 28,658⭐、周下载 **2.56 万**——**dsh 用户基数是 opencli 的 ~19 倍**,增长叙事应是"把 opencli 带给不知道它的 dsh 用户",不是抢既有重叠。

| 假设 | 验证结果 | 对计划的影响 |
|---|---|---|
| R1 审批门有人要 | ✅ **强验证**:榜首 Lum1104(302⭐)issue 队列最密集主题就是审批——"审批三件套(通知+等待+排队)""拒因要清晰""模型要克制用 browser_*" | R1 提级最优先,并吸收其 UX 教训:系统通知、排队、清晰拒因、restricted 策略 |
| R2 适配器创作/分发有人要 | ✅ **强验证**:opencli 100 个 open issue 中 **29 个**是新站点/命令请求(liepin/Zenodo/twitter 多推…),占队列近 1/3 | R2/R4 方向坐实;但**"官方不收"前提被推翻**——近 50 个已合并 PR 延迟 p50=0 天/最长 2 天,72% 合并率。社区仓重定位为**快速通道+实验场+上游前置**,不是"被拒兜底" |
| 我方 PR 挂两月 | 真相:零 CI/零评论/零 review=**掉进缝隙,非被拒** | 行动:rebase + ping;跑 convention-audit 自查后重提 |
| R3 定时/录制有人要 | ⚠️ **无直接信号**(竞品 issue 无人提 cron/录制) | R3 降为"做给自己+种子用户验证",不进发布叙事 |
| 自建浏览器底座是负担 | ✅ 旁证:wqty123 用户全在骂 host 掉线/electron 缺失 | wrapper-over-daemon 路线再次确认正确 |
| R0 可收录 | ✅ 我们的 package.json 已含 dsh.bundle.patch(最常见拒收项) + dsh.client;流程=一个 YAML PR | R0 无结构障碍,随时可发 |

**修正后的优先级**:R0 → R1(吸收审批 UX 教训)→ R2(社区仓重定位为快速通道)→ R3(待自验证)→ R4。
...
### 五·五、安装量数据洞察(2026-08-30,downloads.json 全量 2628 插件/78.2 万总安装)

- **类别安装量排序**:vision 12.3万(91个)> ui 11.8万 > market 10.8万(其中商店本体占8.2万)> tools 6.6万 > notify 4.8万 > memory 4.1万 > … > **browser 2.6万(63个,排第11)**;
- **用户装插件的第一动机 = 补短板**:vision 三件套全部过万(modlens 6.4万/路由 2.2万/toolkit 1.7万)= 看不见图;notify/remote 类(pocket 手机远程 1万/飞书bot 7千/27渠道通知 6.7千)= 够不着手机——都是"扩展 agent 的感官与触达";
- **browser 类的真实头部是 modsearch(8781装,第二名3倍)**:形态是"联网搜索+返回结构化 JSON 证据",**不是浏览器控制**——写操作/办事需求在安装量上尚未被任何插件验证;wqty123 2626、anweat 2430(用 opencli 者)都在长尾;
- **对我们的定位修正**:主卖点对齐被验证的主航道——「**modsearch 的登录态升级版**:同样的结构化 JSON,但用你自己的账号视角、覆盖 173 个垂直站点(B站/知乎/微博/GitHub/小红书/雪球…),并且能反向办事(write,带审批门)」。read 场景是流量入口,write+审批门是差异化纵深;"浏览器办事"不是第一叙事;
- **vision 赛道启示**:91 个插件卷一个需求仍有 3 个过万——高密度需求赛道后来者靠差异化仍能吃;browser 赛道天花板低但我们的形态(登录态结构化数据)实际竞争的是 search/vision 交叉位,不是 63 个 browser 插件的原语位。
