...
var TIMEOUT_MS = 15e3;
var SystemOne = class _SystemOne {
  key;
  endpoint;
  constructor(key, endpoint) {
    this.key = key ?? _SystemOne.loadKey();
    this.endpoint = endpoint ?? ENDPOINT;
  }
  static loadKey() {
    if (process.env.TYPESAFE_API_KEY) return process.env.TYPESAFE_API_KEY;
    try {
      const p = process.platform === "win32" ? String.raw`C:\Users\${process.env.USERNAME ?? ''}\.dsh\typesafe-key` : `${process.env.HOME ?? ""}/.dsh/typesafe-key`;
...
_computedKey = Service.init, _dec = Remote("status"), _dec1 = Remote("adapters"), _dec2 = Remote("refresh"), _dec3 = Remote("daemon-start"), _dec4 = Remote("settings"), _dec5 = Remote("approval-set"), _dec6 = Remote("adapter-disable"), _dec7 = Remote("login-check"), _dec8 = Remote("adapter-detail"), _dec9 = Remote("schedule-add"), _dec10 = Remote("schedule-list"), _dec11 = Remote("ingest-events"), _dec12 = Remote("audit-list"), _dec13 = Remote("logs-tail"), _dec14 = Remote("browser-cdp"), _dec15 = Remote("schedule-run-now"), _dec16 = Remote("schedule-history"), _dec17 = Remote("schedule-toggle"), _dec18 = Remote("schedule-remove"), _dec19 = Remote("try-run"), _dec20 = Remote("replay"), _dec21 = Remote("script-catalog"), _dec22 = Remote("script-run-builtin"), _dec23 = Remote("crawl"), _dec24 = Remote("script-validate"), _dec25 = Remote("userscript-run"), _dec26 = Remote("recipe-run"), _dec27 = Remote("automation-search"), _dec28 = Remote("automation-develop"), _dec29 = Remote("automation-run"), _dec30 = Remote("automation-mode-get"), _dec31 = Remote("automation-mode-set"), _dec32 = Remote("promote-recording"), _dec33 = Remote("install-opencli-skill"), _dec34 = Remote("rulepacks-list"), _dec35 = Remote("rulepacks-set");
        ...
        const results = await Promise.all(sites.map(async (site) => {
          try {
            ...
            return {
              site,
              ok: out.exitCode === 0,
              text: (out.exitCode === 0 ? out.stdout : out.stderr).slice(0, 900)
            };
          } catch (err) {
            return {
              site,
...
${r.text}`).join("\n\n");
        return {
          text: `\u6279\u91CF\u91C7\u96C6 ${okN}/${sites.length} \u7AD9\u6210\u529F:
...
${r.stderr}`.trim();
  ...
  async logsTail() {
    const candidates = [
      join(homedir(), ".opencli", "daemon.log"),
      join(homedir(), ".opencli", "logs", "daemon.log")
    ];
  ...
  async scheduleHistory(p) {
    return {
      ok: true,
      history: this.runHistory[String(p.id ?? "")] ?? []
    };
  }
  ...
  * 任何异常一律放行给 next(),绝不因审批门自身故障阻塞工具。 */
  ...
  async runOpencli(argv, timeoutMs = 6e4, stdoutMaxBytes = 1048576) {
    ...
    try {
      ...
      const raw = runner !== null ? await runner(spec) : {
        exitCode: 1,
        stdout: "dsh shell \u80FD\u529B\u4E0D\u53EF\u7528(\u65E2\u65E0 run \u4E5F\u65E0 execute)"
      };
      const asText = (x) => {
...
${clip(out.stderr, 2e3)}`;
  ...
  async runSiteCommand(siteCmd, id) {
    ...
    for (let attempt = 1; attempt <= attempts; attempt++) {
      ...
      if (lastOk) break;
      if (attempt < attempts) await new Promise((res) => setTimeout(res, 15e3));
    }
