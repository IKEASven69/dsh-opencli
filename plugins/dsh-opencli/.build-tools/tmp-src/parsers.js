 ...
 */ /** 解析 `opencli daemon status` 文本输出。 */ export function parseDaemonStatus(text) {
...
/** 把 `opencli list --format json` 的命令级平铺数组按 site 聚合成适配器列表。 */ export function normalizeAdapterList(input) {
 ...
 * 行形如 `- bilibili (28): search, video, up …` */ export function buildAdapterDirectory(adapters, maxSites = 70, maxCmdsPerSite = 6) {
...
/** 查某条 site 命令的读写权限(审批门用);找不到返回 unknown(按写处理,安全默认)。 */ export function commandAccess(input, adapter, command) {
...
/** 有 whoami 命令的站点列表(登录态巡检用),按命令数降序稳定排序。 */ export function sitesWithWhoami(input, limit = 40) {
    if (!Array.isArray(input)) return [];
    const sites = new Map();
    for (const raw of input){
        if (raw === null || typeof raw !== 'object') continue;
 ...
 * - ask-unknown:权限未知,按写审批(安全默认) */ export function approvalDecision(approvalOn, disabled, toolName, adapter, command, access) {
