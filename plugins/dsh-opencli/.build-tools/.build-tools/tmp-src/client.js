...
/* ── OpenCLIApp 设计令牌(0.1.38 实测)── */ const CSS = `
...
function Panel() {
    ...
    const replayRecording = async (id)=>{
        ...
        for (const step of r.steps){
            ...
            if (cmd.startsWith('browser_')) await rpc('replay', {
                step
            });
            else await rpc('site-replay', {
                step
            });
        }
    };
    ...
    return createElement('div', {
        className: 'ocp'
    }, createElement('style', null, CSS), // ── 页头(图标 + 大标题 + 描述 + 刷新)──
    createElement('div', {
        className: 'ocp-head'
    }, createElement('span', {
        className: 'ocp-icon'
        ...
        className: 'ocp-title'
    }, 'OpenCLI 管理中心'), createElement('div', {
        className: 'ocp-desc'
    }, '浏览内置 OpenCLI 命令;dsh 会话经 site 工具直接调用,缺失站点可让模型现场创作。')), createElement('button', {
        className: 'ocp-btn',
        onClick: reload,
        disabled: busy
    }, busy ? '检测中…' : '刷新/诊断')), // ── L1:普通人 3 击开箱（0 配置，用处→效果→怎么用）──
    createElement('div', {
        className: 'ocp-card'
    }, createElement('div', {
        style: {
    ...
    }, '普通人 3 击开箱（0 配置）'), createElement('div', {
        style: {
            display: 'grid',
    ...
    }, '👁️ 看热榜'), createElement('div', {
        className: 'ocp-hint'
    }, '用处：知乎今天热榜前10'), createElement('div', {
        className: 'ocp-hint'
    }, '效果：结构化榜单转摘要'), createElement('div', {
        className: 'ocp-hint'
    }, '怎么用：对 agent 说“知乎热榜前10摘要”'), createElement('div', {
        className: 'ocp-code',
        style: {
            marginTop: '6px'
        }
    }, 'site zhihu hot')), createElement('div', {
        className: 'ocp-use',
        style: {
            border: '1px solid rgba(255,255,255,.06)',
            borderRadius: '12px',
    ...
    }, '🔍 搜视频'), createElement('div', {
        className: 'ocp-hint'
    }, '用处：B站搜罗翔'), createElement('div', {
        className: 'ocp-hint'
    }, '效果：播放量前3 + 链接'), createElement('div', {
        className: 'ocp-hint'
    }, '怎么用：搜“bilibili”→ 点 search'), createElement('div', {
        className: 'ocp-code',
        style: {
            marginTop: '6px'
        }
    }, 'site bilibili search 罗翔')), createElement('div', {
        className: 'ocp-use',
        style: {
            border: '1px solid rgba(255,255,255,.06)',
            borderRadius: '12px',
    ...
    }, '📤 发动态'), createElement('div', {
        className: 'ocp-hint'
    }, '用处：把这个发我微博'), createElement('div', {
        className: 'ocp-hint'
    }, '效果：在你的号真实发布'), createElement('div', {
        className: 'ocp-hint'
    }, '怎么用：说“发我微博”→ 审批窗点允许'), createElement('div', {
        className: 'ocp-code',
        style: {
            marginTop: '6px'
        }
    }, 'site weibo post …')))), // ── 未安装:安装引导卡(第一职责;可折叠)──
    missing ? createElement('div', {
    ...
    }, '未检测到 opencli —— 三步接入'), createElement('span', {
        ...
        className: 'ocp-code'
    }, 'DSH_OPENCLI_BIN'), '。')))) : null) : null, // ── 状态卡(系统页样式)──
    status !== null && status.ok ? createElement('div', {
        ...
        className: 'ocp-sk'
    }, 'daemon'), createElement('span', {
        className: 'ocp-sv'
    }, up ? '运行中' : '未运行'), up ? null : createElement('button', {
        className: 'ocp-btn ocp-btn-sm',
        disabled: starting,
        onClick: ()=>{
            void startDaemon();
        }
    }, starting ? '启动中…' : '启动 daemon'), daemonMsg !== null ? createElement('span', {
        className: 'ocp-err'
        ...
        className: 'ocp-sk'
    }, 'Chrome 扩展'), createElement('span', {
        className: 'ocp-sv'
    }, d?.extension === 'connected' ? '已连接' : d?.extension ?? '未知')), createElement('div', {
        className: 'ocp-srow'
    }, createElement('span', {
        className: 'ocp-sk'
    }, 'write 审批门'), createElement('span', {
        className: 'ocp-sv'
    }, settings === null ? '…' : settings.approvalOn ? '开启(site 写操作先经确认)' : '关闭'), createElement('button', {
        className: 'ocp-btn ocp-btn-sm',
        onClick: ()=>{
            void setApproval(!(settings?.approvalOn ?? true));
        }
    }, settings?.approvalOn === false ? '开启' : '关闭')), whoamiCount > 0 ? createElement('div', {
        className: 'ocp-srow'
    }, createElement('span', {
        className: 'ocp-sk'
    }, '登录态'), createElement('span', {
        className: 'ocp-sv'
    }, `${whoamiCount} 个站点可巡检`), createElement('button', {
        className: 'ocp-btn ocp-btn-sm',
        disabled: checking,
        onClick: ()=>{
            void runLoginCheck();
        }
    }, checking ? '巡检中…' : '巡检登录态')) : null, createElement('div', {
        className: 'ocp-srow'
    }, createElement('span', {
        className: 'ocp-sk'
    }, '环境'), createElement('span', {
        className: 'ocp-sv ocp-mono',
        style: {
            fontSize: 12
        }
    }, `v${status.version ?? '?'}`, ` · ${status.adapterSites ?? adapters?.length ?? '?'} 站点`, ` · ${siteCmds + appCmds} 命令`, d?.port !== undefined ? ` · 端口 ${d.port}` : '', d?.uptime !== undefined ? ` · ↑ ${d.uptime}` : ''))) : null, // ── 登录态巡检结果卡 ──
    login !== null ? createElement('div', {
        className: 'ocp-card',
        style: {
        ...
        className: 'ocp-setup-t'
    }, '登录态巡检'), login.ok ? createElement('span', {
        className: 'ocp-hint'
    }, `${login.results.filter((r)=>r.ok).length}/${login.results.length} 在线 · ${new Date(login.checkedAt ?? '').toLocaleTimeString()}`) : null, createElement('button', {
        className: 'ocp-btn ocp-btn-sm',
        onClick: ()=>{
            setLogin(null);
        }
    }, '收起')), login.ok ? createElement('div', {
        className: 'ocp-login'
    }, login.results.map((r)=>createElement('div', {
            key: r.site,
            className: 'ocp-lrow',
            title: r.detail ?? ''
        }, createElement('span', {
        ...
        }, r.timedOut ? '探测超时' : r.detail ?? '')))) : createElement('div', {
        className: 'ocp-err'
    }, login.error ?? '巡检失败')) : null, // ── 录制回放 / 我的适配器 / 定时订阅（Stage2）──
    createElement('div', {
        className: 'ocp-card',
        style: {
            padding: '12px 18px'
        ...
        className: 'ocp-setup-t'
    }, '录制回放'), createElement('span', {
        className: 'ocp-hint'
    }, `${recordings.length} 条`), createElement('button', {
        className: 'ocp-btn ocp-btn-sm',
        onClick: ()=>{
            if (isRecording) stopRecording();
            else startRecording();
        }
    }, isRecording ? '停止录制' : '开始录制')), isRecording ? createElement('div', {
        style: {
            display: 'flex',
        ...
        placeholder: '录制名称（如：每日知乎热榜）',
        value: recordName,
        onChange: (e)=>setRecordName(e.target.value)
    }), createElement('span', {
        className: 'ocp-hint'
    }, `已录 ${recordSteps.length} 步`)), createElement('div', {
        className: 'ocp-srow'
    }, createElement('input', {
        className: 'ocp-input',
        style: {
        ...
        placeholder: '添加步骤（回车确认，如：site zhihu hot）',
        onKeyDown: (e)=>{
        ...
        style: {
            ...
            className: 'ocp-hint'
        }, `${r.steps.length}步 · ${new Date(r.createdAt).toLocaleDateString()}`), createElement('button', {
            className: 'ocp-btn ocp-btn-sm',
            onClick: ()=>{
                void replayRecording(r.id);
            }
        }, '回放'), createElement('button', {
            className: 'ocp-btn ocp-btn-sm',
            onClick: ()=>{
                persistRecordings(recordings.filter((x)=>x.id !== r.id));
            }
        }, '删除')))) : createElement('div', {
        className: 'ocp-hint',
        style: {
            padding: '8px 0'
        }
    }, '暂无录制。点击“开始录制”后，你的 browser_* / site 调用会自动追加为步骤（MVP：手动在下方输入步骤）'), createElement('div', {
        className: 'ocp-srow',
        style: {
        ...
        className: 'ocp-setup-t'
    }, '定时订阅'), createElement('span', {
        className: 'ocp-hint'
    }, 'dsh.schedule')), createElement('div', {
        className: 'ocp-srow'
    }, createElement('input', {
        className: 'ocp-input',
        style: {
        ...
        placeholder: 'site 命令（如：zhihu hot）',
        value: scheduleSite,
        ...
        className: 'ocp-hint'
    }, '创建后可在 dsh schedule list 查看。本地适配器可在下方“Site命令”中通过禁用/启用管理，即“我的适配器”。')), // ── 高级自动化（脚本/配方/泛爬，默认收起）──
    createElement('details', {
        className: 'ocp-adv',
        open: false
    }, createElement('summary', null, '高级自动化（脚本/配方/泛爬）— 给需要的人'), createElement('div', {
        style: {
            display: 'flex',
            flexDirection: 'column',
            gap: '12px',
        ...
        value: 'read-only'
    }, '只读'), createElement('option', {
        value: 'standard'
    }, '标准（默认）'), createElement('option', {
        value: 'autonomous'
    }, '自主'), createElement('option', {
        value: 'unrestricted'
    }, '无人值守'))), createElement('div', {
        className: 'ocp-srow'
    }, createElement('span', {
        className: 'ocp-sk'
    }, '限流'), createElement('span', {
        className: 'ocp-hint'
    }, 'minDelay 750ms / 并发 2 / 突发 3 / 冷却 30s'), createElement('span', {
        className: 'ocp-tag ocp-tag-read',
        style: {
            marginLeft: 'auto'
        }
    }, '已启用')), createElement('div', {
        className: 'ocp-srow'
    }, createElement('span', {
        className: 'ocp-sk'
    }, '限域登录'), createElement('span', {
        className: 'ocp-hint'
    }, 'authProfiles: allowedDomains 限域，默认只读不回写')), createElement('div', {
        style: {
            display: 'grid',
    ...
    adapters !== null ? createElement('div', {
    ...
    }, 'Site 命令', createElement('b', null, String(siteCmds))), createElement('button', {
    ...
    }, 'App 命令', createElement('b', null, String(appCmds)))), createElement('div', {
        ...
        className: 'ocp-count'
    }, createElement('b', null, String(q.length > 0 ? filtered.length : tab === 'site' ? siteCmds : appCmds)), ' 条命令')), createElement('div', {
        className: 'ocp-sites'
        ...
        }, createElement('span', null, 'dsh 会话调用:'), createElement('span', {
            className: 'ocp-code'
        }, `site ${a.name} <命令>`), createElement('span', null, '· 点击行复制完整命令'), createElement('button', {
            className: 'ocp-btn ocp-btn-sm',
            style: {
                marginLeft: 'auto'
            },
        ...
        }, a.disabled === true ? '启用' : '禁用')), detail === undefined ? createElement('div', {
            className: 'ocp-load'
