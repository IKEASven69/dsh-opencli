...
/** 支持导入的扩展名(小写)。 */ export const TEXT_EXTS = new Set([
    '.md',
    '.markdown',
    '.txt',
    '.log',
...
export const PDF_EXTS = new Set([
    '.pdf'
]);
export const DOCX_EXTS = new Set([
    '.docx'
]);
...
/** 单文件原始大小上限(字节),防御性截流。 */ export const MAX_FILE_BYTES = 8 * 1024 * 1024;
/** 极简 HTML 正文抽取:剥脚本/样式/导航结构,块级标签转空行,解码常见实体。 */ export function htmlToText(html) {
...
/** 抽取结果:null 表示不支持/失败,reason 供注册表 error 字段;meta 为 frontmatter 标量(仅文本族)。 */ export async function extractText(path) {
    ...
    try {
        ...
        if (TEXT_EXTS.has(ext)) {
            ...
            return {
                ...
                rawHash,
                meta,
                reason: text === null ? '空文件' : undefined
            };
        }
        if (PDF_EXTS.has(ext)) {
