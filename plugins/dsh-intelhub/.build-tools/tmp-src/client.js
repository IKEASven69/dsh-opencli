...
function Panel() {
    ...
    /** 浏览器端读取文本族文件(pdf/docx 走路径导入)。 */ const readFiles = async (fl)=>{
        const out = [];
    ...
    const refresh = async ()=>{
        ...
        if (d.ok && d.value !== undefined) setDash(d.value);
    };
    useEffect(()=>{
        void refresh();
    }, []);
    ...
    const hitLines = (results?.hits ?? []).map((h, i)=>{
        ...
        }, h.ref), createElement('span', {
            className: 'zkb-hitscore'
        }, `相关度 ${bar}%`)), createElement('div', {
            className: 'zkb-hitsnip'
        }, h.text.length > 260 ? h.text.slice(0, 260) + '…' : h.text));
    });
    return createElement('div', {
        className: 'zkb'
    }, createElement('style', null, CSS), // ── 页头 ──
    createElement('div', {
        className: 'zkb-head'
    }, createElement('span', {
        className: 'zkb-icon'
    }, 'IH'), createElement('div', {
        style: {
            minWidth: 0
        }
    }, createElement('div', {
        className: 'zkb-title'
    }, '情报站 IntelHub'), createElement('div', {
        className: 'zkb-desc'
    }, '刷到的信息自动沉淀为可检索的知识:采集/文件夹/网页/笔记 → 语义+关键词混合检索带出处 → Obsidian 反哺。零守护进程·零 API key·文档不出本机。'))), err !== null ? createElement('div', {
        className: 'zkb-err'
    }, err) : null, // ── 仪表盘卡 ──
    dash !== null && dash.ok ? createElement('div', {
        className: 'zkb-card'
    }, createElement('div', {
        className: 'zkb-srow'
    }, createElement('span', {
        className: 'zkb-sk'
    }, '知识库仪表盘'), createElement('span', {
        className: 'zkb-badge',
        style: {
            marginLeft: 'auto'
        }
    }, `今日 +${dash.totals.todayFiles}`), dash.totals.rawPending > 0 ? createElement('span', {
        className: 'zkb-badge',
        style: {
            background: 'rgba(255,165,61,0.15)',
            color: '#E8A33D'
        }
    }, `待分诊 ${dash.totals.rawPending}`) : null), // 14 天采集趋势(SVG 迷你柱状)
    createElement('div', {
        style: {
            display: 'flex',
            alignItems: 'flex-end',
        ...
        return dash.daily.map((d)=>createElement('div', {
                key: d.date,
                title: `${d.date} · ${d.files} 篇 / ${d.chunks} 块`,
                style: {
                    flex: 1,
                    display: 'flex',
            ...
            }, d.chunks > 0 ? String(d.chunks) : ''), createElement('div', {
                style: {
                    width: '70%',
    ...
    createElement('div', {
        ...
        return dash.bySrc.map((x, i)=>createElement('div', {
                key: x.k,
                title: `${x.k} · ${x.chunks} 块`,
                style: {
        ...
        }, '■ ', colors[i % colors.length] === '#4A9EFF' ? '' : '', `${x.k} · ${x.chunks}块`);
    }))), // TOP 作者榜
    dash.topAuthors.length > 0 ? createElement('div', {
        style: {
            padding: '0 4px 10px'
        }
    }, createElement('div', {
    ...
    }, 'TOP 作者(按最高赞)'), ...dash.topAuthors.slice(0, 5).map((a, i)=>createElement('div', {
            key: a.author,
            style: {
                display: 'flex',
                alignItems: 'center',
        ...
        }, a.author), createElement('span', {
            style: {
                fontSize: '11px',
                color: '#9A9EA8'
            }
        }, `${a.files} 篇 · ♥${a.likes}`)))) : null) : null, // ── 状态卡 ──
    createElement('div', {
        className: 'zkb-card'
    }, createElement('div', {
        className: 'zkb-srow'
        ...
        className: 'zkb-sk'
    }, '已导入'), createElement('span', {
        className: 'zkb-sv'
    }, `${status?.files ?? '–'} 个文件 · ${status?.chunks ?? '–'} 块`), status !== null && status.indexing > 0 ? createElement('span', {
        className: 'zkb-badge'
    }, `索引中 ${status.indexing}`) : null), createElement('div', {
        className: 'zkb-srow'
    }, createElement('span', {
        className: 'zkb-sk'
    }, '检索模型'), createElement('span', {
        className: `zkb-dot ${modelDot}`
    }), createElement('span', {
        className: 'zkb-sv'
    }, modelText), createElement('span', {
        className: 'zkb-hint',
        style: {
            marginLeft: 'auto'
        }
    }, '首次导入时自动下载(约 30MB),全程本机'))), // ── 空态演示卡(知识库为空时)──
    status === null || status.files === 0 && status.chunks === 0 && status.indexing === 0 ? createElement('div', {
        className: 'zkb-card'
    }, createElement('div', {
        className: 'zkb-srow'
    }, createElement('span', {
        className: 'zkb-sk'
    }, '第一次用?'), createElement('span', {
        className: 'zkb-hint',
        style: {
            flex: 1
        }
    }, '30 秒看懂语义检索和关键词检索的差别——导入一份样例手册,问一个文档里"没写过"的问题。'), createElement('button', {
        className: 'zkb-btn zkb-btn-pri',
        disabled: busyDemo,
        onClick: ()=>{
            void doDemo();
        }
    }, busyDemo ? '准备中…' : '看演示')), demo !== null && demo.ok ? createElement('div', {
        className: 'zkb-srow',
        style: {
            display: 'block'
        }
        ...
        className: 'zkb-hint'
    }, `查询「${demo.query}」——样例手册里写的是"30 秒内无响应则会话中断",没有"超时"二字:`), createElement('div', {
        style: {
            marginTop: 8
        }
    }, createElement('span', {
        className: 'zkb-demo-tag zkb-demo-no'
    }, `关键词检索 ${demo.fts.length} 条`), demo.fts.length > 0 ? createElement('span', {
        className: 'zkb-hint'
    }, demo.fts.map((h)=>h.ref).join(' ')) : null), createElement('div', {
        style: {
            marginTop: 6
        }
    }, createElement('span', {
        className: 'zkb-demo-tag zkb-demo-yes'
    }, `语义混合检索 ${demo.hybrid.length} 条`), demo.hybrid.slice(0, 2).map((h, i)=>createElement('div', {
            key: i,
            style: {
                marginTop: 6
            }
            ...
            className: 'zkb-hitref'
        }, h.ref), createElement('div', {
            className: 'zkb-hitsnip'
        }, h.text.length > 120 ? h.text.slice(0, 120) + '…' : h.text))))) : null) : null, // ── 导入卡 ──
    createElement('div', {
        className: 'zkb-card'
    }, createElement('div', {
        className: 'zkb-srow'
        ...
        onDrop: (e)=>{
            e.preventDefault();
            setDragOn(false);
            if (e.dataTransfer?.files !== undefined && e.dataTransfer.files.length > 0) void doUpload(e.dataTransfer.files);
        }
    }, '拖入文件(可多选,md/txt/json/代码)或点击选择'), createElement('input', {
        className: 'zkb-input',
    ...
    }, busyImport ? '导入中…' : '导入')), createElement('div', {
        className: 'zkb-srow'
    }, createElement('span', {
        className: 'zkb-hint'
    }, 'Obsidian 库直接填库目录(自动跳过 .obsidian 内部文件);网页让 agent 用 kb_import_url 存档;对话里的长文/微博等社交内容由 agent 经 kb_note 写入。重复导入只处理新增与变更。'))), // ── 检索预览卡 ──
    createElement('div', {
        className: 'zkb-card'
    }, createElement('div', {
        className: 'zkb-srow'
    ...
    }, busySearch ? '检索中…' : '检索')), createElement('div', {
        className: 'zkb-srow'
    }, createElement('input', {
        className: 'zkb-input',
    ...
    })), results !== null && results.hits.length > 0 ? createElement('div', {
        className: 'zkb-srow',
        style: {
            display: 'block'
        }
    }, ...hitLines) : results !== null ? createElement('div', {
        className: 'zkb-srow'
    }, createElement('span', {
        className: 'zkb-hint'
    }, `没有匹配${results.note !== undefined && results.note !== '' ? `(${results.note})` : ''}`)) : null), // ── workspace 常驻目录卡 ──
    createElement('div', {
        className: 'zkb-card'
    }, createElement('div', {
        className: 'zkb-srow'
        ...
        className: 'zkb-sk'
    }, '常驻目录'), createElement('span', {
        className: 'zkb-hint'
    }, '注册后自动跟随:采集脚本落盘 → 增量索引 → 即刻可问')), ...workspaces.map((w)=>createElement('div', {
            className: 'zkb-frow',
            key: w.path
        }, createElement('span', {
            ...
            className: 'zkb-fpath',
            title: w.path
        }, w.label + ' · ' + w.path), createElement('button', {
            className: 'zkb-btn',
            style: {
                padding: '4px 10px',
                fontSize: '11px'
    ...
    createElement('div', {
        ...
        className: 'zkb-sk'
    }, '定时任务'), createElement('span', {
        className: 'zkb-hint'
    }, '重启不丢;agent 可经 kb_schedule 设置(如按发博节奏定时扫描)')), ...schedules.map((sc)=>createElement('div', {
            className: 'zkb-frow',
            key: sc.name
        }, createElement('span', {
            ...
            className: 'zkb-fpath'
        }, sc.name + ' · ' + (sc.kind === 'daily' ? `每天 ${sc.at ?? ''}` : `每 ${sc.everyMin ?? '?'} 分钟`) + ' · scan'), createElement('button', {
            className: 'zkb-btn',
            style: {
                padding: '4px 10px',
                fontSize: '11px'
        ...
        }, sc.enabled === true ? '停用' : '启用'), createElement('button', {
            className: 'zkb-btn',
            style: {
                padding: '4px 10px',
                fontSize: '11px'
    ...
    createElement('div', {
        ...
        className: 'zkb-sk'
    }, '已导入文件'), createElement('span', {
        className: 'zkb-hint',
        style: {
            marginLeft: 'auto'
        }
    }, '删除即从索引移除,原文件不受影响')), (list?.files ?? []).length === 0 ? createElement('div', {
        className: 'zkb-srow'
    }, createElement('span', {
        className: 'zkb-hint'
    }, '还没有导入文件。')) : createElement('div', {
        className: 'zkb-files'
    }, ...(list?.files ?? []).map((f)=>createElement('div', {
            className: 'zkb-frow',
            key: f.path
        }, createElement('span', {
            ...
            className: 'zkb-fpath',
            title: f.error !== undefined ? f.error : f.path
        }, f.path), createElement('span', {
            className: 'zkb-fmeta'
        }, f.status === 'done' ? `${f.chunks} 块` : f.status === 'indexing' ? '索引中' : '失败'), createElement('button', {
            className: 'zkb-btn',
            style: {
                padding: '4px 10px',
                fontSize: '11px'
...
/** 侧边栏精简版:检索 + 来源结果(完整管理在设置页)。 */ function SidebarKB() {
    const [query, setQuery] = useState('');
    const [results, setResults] = useState(null);
    ...
    const hits = (results?.hits ?? []).map((h, i)=>createElement('div', {
            key: i,
            style: {
                padding: '8px 0',
