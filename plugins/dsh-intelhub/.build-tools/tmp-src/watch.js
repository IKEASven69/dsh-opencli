...
const normKey = (p)=>p.replace(/[\\/]+/g, '/').replace(/\/$/, '').toLowerCase();
export class WorkspaceManager {
    ...
    debounces = new Map();
    loaded = false;
    constructor(homeDir, trigger, debounceMs = 4000, scanMs = 5 * 60 * 1000){
    ...
    async load() {
        ...
        this.loaded = true;
        try {
            const raw = JSON.parse(await readFile(join(this.homeDir, 'workspaces.json'), 'utf8'));
    ...
    /** 事件防抖:静默 debounceMs 后触发一次增量导入。 */ schedule(key) {
