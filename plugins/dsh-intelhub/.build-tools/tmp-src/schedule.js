...
const normKey = (n)=>n.trim().toLowerCase();
...
export class ScheduleManager {
    ...
    async load() {
        ...
        this.loaded = true;
        try {
            const raw = JSON.parse(await readFile(join(this.homeDir, 'schedules.json'), 'utf8'));
    ...
    async persist() {
        ...
        await writeFile(join(this.homeDir, 'schedules.json'), JSON.stringify({
            version: 1,
            schedules: [
                ...this.entries.values()
            ]
    ...
    /** 到点执行并排下一次;interval 用自校准 setTimeout(防漂移),daily 算到明天的 HH:MM。 */ startTimer(e) {
        const key = normKey(e.name);
        const prev = this.timers.get(key);
        if (prev !== undefined) clearTimeout(prev);
        const delayMs = e.kind === 'interval' ? Math.max(1, e.everyMin ?? 60) * 60_000 : this.msUntilTodayOrTomorrow(e.at ?? '09:00');
        const t = setTimeout(()=>{
            void (async ()=>{
